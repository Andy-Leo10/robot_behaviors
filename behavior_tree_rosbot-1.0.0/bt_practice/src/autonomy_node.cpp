 ...
 * Main behavior node
 ...
 Script that tests the `autonomy_node.cpp` script to execute the Behavior Tree.
 ros2:      ros2 run bt_practice autonomy_node_cpp --ros-args -p target_color:=red
 */
...
#include "nav2_behavior.h"
#include "vision_behavior.h"

using namespace std::chrono_literals;

const std::string default_bt_xml_file = 
    ament_index_cpp::get_package_share_directory("bt_practice") + "/bt_xml/tree_naive.xml";
const std::string default_location_file =
    ament_index_cpp::get_package_share_directory("bt_practice") + "/config/locations.yaml";
