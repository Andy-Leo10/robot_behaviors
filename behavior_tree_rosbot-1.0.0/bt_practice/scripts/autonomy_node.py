...
Example usage:
  ros2 run bt_practice autonomy_node.py
  ros2 run bt_practice autonomy_node.py --ros-args -p location_file:=/path/to/my/file.yaml
  ros2 run bt_practice autonomy_node.py --ros-args -p tree_type:=queue -p target_color:=red
"""
...
from behaviors.navigation import GoToPose, GetLocationFromQueue
from behaviors.vision import LookForObject


default_location_file = os.path.join(
    get_package_share_directory("bt_practice"), "config", "locations.yaml"
)
...
class AutonomyBehavior(Node):
    def __init__(self):
        ...
        self.declare_parameter("target_color", value="red")
    ...
    def create_queue_tree(self):
        ...
        seq.add_children(
            [
                ...
                GoToPose("go_to_location", tree.node),
            ]
        )
        if self.enable_vision:
            seq.add_child(
